...
# Cache connections to the broker in a thread-local
_thread_local = threading.local()


def publish(topic, headers=None, body=None, timeout=10):
    """
    ...
    Args:
        ...
        timeout (float, int): The time to wait, in seconds, for a response
            from the fedmsg broker. Set to 0 for infinite timeout.
    ...
    # Block forever, don't multiply by 1000 to make it seconds
    if not timeout == -1:
        timeout *= 1000
